...
class PrefixTree:
    ...
    def insert(self, word: str) -> None:
        ...
        for i in range(len(word)):
            c = word[i]
            c_s = word[i] + self.sentinel
            flag = i == len(word) - 1
            insert = c if not flag else c_s

            if c not in last and c_s not in last:
                last[insert] = {}

            # check if we need to change to sentinel
            # e.g. inserting "app" with {'a': {'p': {'p': {'l': {'e#': {}}}}}}
            if flag and c in last:
                ref = last[c]
                del last[c]
                last[insert] = ref
    ...
    def startsWith(self, prefix: str) -> bool:
        ...
        for i in range(len(prefix)):
            c = prefix[i]
            search = prefix[i] + self.sentinel

            if c not in last and search not in last:
                return False
