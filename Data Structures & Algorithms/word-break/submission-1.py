...
class Solution:
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        ...
        def solve(i: int) -> bool:
            ...
            # attempt to use every word in wordDict
            for word in wordDict:
                # if the substring isnt in words,
                # we continue iterating, until
                if s[i:].startswith(word) and solve(i + len(word)):
                    return True
            
            # we've tried every word and havent returned true
            # so we cannot find the right word
            return False
