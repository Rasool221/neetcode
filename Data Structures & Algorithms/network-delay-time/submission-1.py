...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        while h:
            # print(f"before: {h=}, {dist=}")

            # getting top element of heap
            top: tuple[int, int] = heapq.heappop(h)

            t_dist = top[0] # distance to current node
            t_node_id = top[1] # id of current node

            # if we've already got a shorter path
            # for this element, we can skip processing it
            if t_dist > dist[t_node_id - 1]:
                continue

            # print(f"{t_node_id=}, {t_dist=}")
            ...
            for d, i in g[t_node_id]:
                ...
                # print(f"{i=}, {d=}, {distance=}")

                # if a shorter path to this
                # neighbor node is found, update it
                if distance < dist[i - 1]:
                    dist[i - 1] = t_dist + d
                    heapq.heappush(h, (distance, i))

            # print(f"after {h=}, {dist=}")
        
        # fetch the answer (max time we've recorded = shortest time to reach all nodes)
        # if any element is float('inf') that means we didnt pick up all elements
        max_d: int = float('-inf')
