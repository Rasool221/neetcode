...
# what if calc 2 dp arrays, one where we rob 1st house (include first house, but skip last house)
# and one where we ignore first house and include last house?
# then max the value of both to be our answer.
# both cases avoid our constraint of not robbing first AND last house 
class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        return max(
            do_dp(nums[:-1]), # skip last house
            do_dp(nums[1:])   # skip first house
        )
